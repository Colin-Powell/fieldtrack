    ...
    try {
      await emailService.sendEmail(
        ...
        `<div style="font-family:Arial,sans-serif;color:#333;padding:20px">
          <h2 style="color:#16A34A">Welcome to FieldTrack!</h2>
          <p>Hello ${firstName},</p>
      ...
      console.error('Failed to send welcome email to student:', err);
    }
...
export async function createSupervisor(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      console.error('Failed to send welcome email to supervisor:', err);
    }
...
export async function resetUserPassword(req: Request, res: Response) {
  try {
    ...
    try {
      await emailService.sendEmail(
        existingUser.email,
        'FieldTrack - Password Reset',
        `<div style="font-family:Arial,sans-serif;color:#333;padding:20px">
          <h2 style="color:#16A34A">Password Reset Complete</h2>
          <p>Hello ${existingUser.name},</p>
          <p>Your password has been reset by an administrator.</p>
          ${!newPassword ? `<p>Your new temporary password is: <strong>${passwordToUse}</strong></p>
          <p>Please log in and change your password as soon as possible.</p>` : '<p>Your password was changed to a new secure value.</p>'}
        </div>`
      );
    } catch (err) {
