...
export async function getAdminDashboard(req: Request, res: Response) {
  try {
    ...
    switch (period) {
      case 'Today': periodStart = new Date(currentDate.setHours(0,0,0,0)); break;
      case 'This Week': {
        const d = new Date();
        const day = d.getDay();
      ...
      case 'This Month': periodStart = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1); break;
      case 'This Year': periodStart = new Date(currentDate.getFullYear(), 0, 1); break;
      case 'All Time': periodStart = undefined; break;
      default: {
    ...
    const activityTrend: { label: string; value: number; dateLabel: string }[] = [];
    const attendanceTrend: { label: string; value: number; dateLabel: string }[] = [];
    ...
    const sysActivityIcons: Record<string, { icon: string; color: string }> = {
