...
async function hasSentDigest(recipientId, type) {
...
async function markDigestSent(recipientId, type, message) {
    const dateStr = getTodayString();
    const title = `${type} - ${dateStr}`;
...
export async function sendSupervisorDailyBriefing() {
    ...
    for (const supervisor of supervisors) {
        ...
        if (!profile)
            continue;
        if (await hasSentDigest(supervisor.id, 'Daily Supervisor Digest'))
            continue;
...
export async function sendStudentDailyBriefing() {
    ...
    for (const student of students) {
        if (await hasSentDigest(student.id, 'Daily Student Digest'))
            continue;
...
export async function sendAdminDailyBriefing() {
    ...
    for (const admin of admins) {
        if (await hasSentDigest(admin.id, 'Daily Admin Digest'))
            continue;
