...
async function sendStudentNotification(studentId, title, message, preferences) {
...
async function hasRecentStudentReminder(studentId, title, lookbackHours) {
...
export async function checkStudentActivityReminders() {
    ...
    for (const activity of activities) {
        ...
        if (studentPrefs?.notifNewActivity === false)
            continue;
...
export async function checkStudentCheckInReminders() {
    ...
    for (const session of sessions) {
        ...
        const lastPing = session.locationPings?.[0]?.timestamp;
        const studentPrefs = student.preferences;
        if (studentPrefs?.notifCheckInOut === false)
            continue;
        if (!lastPing || lastPing >= cutoff)
            continue;
...
export async function checkStudentSubmissionReminders() {
    ...
    for (const submission of submissions) {
        ...
        const studentPrefs = student.preferences;
        if (studentPrefs?.notifReview === false)
            continue;
...
async function checkAppVersionUpdate() {
    try {
        ...
        const lastNotifiedVersion = lastVersionSetting?.value?.version;
