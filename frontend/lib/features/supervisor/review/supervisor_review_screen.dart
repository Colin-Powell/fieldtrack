...
class SupervisorReviewScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SupervisorReviewScreen> createState() =>
      _SupervisorReviewScreenState();
}

class _SupervisorReviewScreenState
    extends ConsumerState<SupervisorReviewScreen> {
  // --- State Variables ---
  double _rating = 0.0;
  ...
  Future<void> _submitFeedback() async {
    if (_commentController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please enter your feedback comments before submitting.',
          ),
        ...
        await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'You are about to mark this activity as "$_selectedStatus".',
                ),
                ...
                const Text(
                  ...
                  style: TextStyle(color: _Colors.red, fontSize: 12),
                ),
              ],
            ),
            actions: [
    ...
    try {
      ...
      if (_selectedStatus == 'Request Revision')
        mappedStatus = 'REVISION_REQUESTED';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Failed to submit review: ${ErrorHandler.getFriendlyErrorMessage(e)}',
            ),
            backgroundColor: _Colors.red,
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    final activity = activityResult is Success
        ? (activityResult as Success).data
        : null;
    ...
    if (activity != null) {
      if (activity['timestamp'] != null) {
        subTime = DateFormat(
          'hh:mm a',
        ).format(DateTime.parse(activity['timestamp']).toLocal());
      }
    ...
    if (activity != null && activity['reviews'] != null) {
      ...
      if (reviews.isNotEmpty) {
        ...
        final revDateStr = lastReview['createdAt'] != null
            ? DateFormat(
                'dd MMM yyyy',
              ).format(DateTime.parse(lastReview['createdAt']).toLocal())
            : '';
      ...
      builder: (context, constraints) {
              ...
              children: [
                ...
                Row(
                  children: List.generate(
                    5,
                    (index) => _buildInteractiveStar(index),
                  ),
                ),
              ],
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStatusOption(
                    ...
                    onTap: () =>
                        setState(() => _selectedStatus = 'Request Revision'),
                  ),
                  const SizedBox(height: 16),
                  _buildStatusOption(
                    title: 'Reject',
        ...
        if (isMobile) {
          return SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [leftContent, const SizedBox(height: 24), rightContent],
            ),
          );
        }
